...
public class FloodFunction {
    ...
    public FloodFunction(Driver newDriver) {
        ...
        flood(colorOfCoord(new Coord(0,0)));
    }
    
    // flood function is to be implemented by students
    public void flood(int colorToFlood) {
        ListIterator<Coord> curr = floodList.listIterator();
        while (curr.hasNext()) {
            Coord currElem = curr.next();

            if (inbound(up(currElem))
                    && colorOfCoord(up(currElem)) == colorToFlood
                    && !floodList.contains(up(currElem))) {
                curr.add(up(currElem));
                curr = floodList.listIterator();
            }
            if (inbound(left(currElem))
                    && colorOfCoord(left(currElem)) == colorToFlood
                    && !floodList.contains(left(currElem))) {
                curr.add(left(currElem));
                curr = floodList.listIterator();
            }
            if (inbound(down(currElem))
                    && colorOfCoord(down(currElem)) == colorToFlood
                    && !floodList.contains(down(currElem))) {
                curr.add(down(currElem));
                curr = floodList.listIterator();
            }
            if (inbound(right(currElem))
                    && colorOfCoord(right(currElem)) == colorToFlood
                    && !floodList.contains(right(currElem))) {
                curr.add(right(currElem));
                curr = floodList.listIterator();
            }
        }
    }

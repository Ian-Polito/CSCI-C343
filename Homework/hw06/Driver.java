...
public class Driver {
    ...
    // Cycles through all the available colors, returns each current chosen color int
    public int cycleColors() {
        //System.out.println("Current chosen color: " + Constants.currColor);
        if (Constants.currColor == 5) {
            Constants.currColor = 0;
        }
        else {
            Constants.currColor += 1;
        }
        return Constants.currColor;
    }

    public void autoPlay() {
        javax.swing.Timer timer = new Timer(Constants.TIMER_WAIT, e -> this.flood(cycleColors()));
        timer.start();
    }
